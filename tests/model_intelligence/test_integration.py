...
class TestModelRuntimeAdapter:
    ...
    def test_compare_runtimes(self):
        adapter = ModelRuntimeAdapter()
        profile = adapter._profiler.get_profile("mistral-7b")
        assert profile is not None
    ...
    def test_get_best_configuration(self):
        adapter = ModelRuntimeAdapter()
        profile = adapter._profiler.get_profile("codellama-7b")
        assert profile is not None
        config = adapter.get_best_configuration(profile, None)
        assert config["model_id"] == "codellama-7b"
...
class TestModelMemoryAdapter:
    ...
    def test_knowledge_graph_relations(self):
        ...
        adapter.record_model_for_task("deepseek-coder-16b", "debug", True)
        adapter.record_outperformance("qwen3-coder-30b", "codellama-7b", "code review")
