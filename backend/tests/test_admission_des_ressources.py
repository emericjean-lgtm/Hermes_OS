...
def test_le_moniteur_systeme_lit_le_compteur_par_processus():
    """`GPU Adapter Memory` mesurait 3,99 Gio quand `GPU Process Memory`
    en mesurait 12,70 sur la même carte — sous-déclaration d'un facteur
    trois, dans le sens qui fait croire qu'il reste de la place."""
    source = io.open(RACINE / "backend/monitoring/gpu_monitor.py",
                     encoding="utf-8").read()
    assert "GPU Process Memory(*)" in source
    assert "GPU Adapter Memory(*)" not in source, (
        "le compteur par adaptateur est de retour : il sous-déclare")


def test_le_banc_et_le_moniteur_lisent_le_meme_compteur():
    """Deux lectures de la même grandeur qui divergent, c'est la situation
    que A-12 nomme. `model_bench` lisait déjà le bon."""
    banc = io.open(RACINE / "backend/model_intelligence/model_bench.py",
                   encoding="utf-8").read()
    moniteur = io.open(RACINE / "backend/monitoring/gpu_monitor.py",
                       encoding="utf-8").read()
    assert "GPU Process Memory" in banc and "GPU Process Memory" in moniteur
