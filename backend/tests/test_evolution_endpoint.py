...
def test_process_task_extracts_skill_on_done(client):
    ...
    response = client.post(f"/evolution/process/{task['id']}")
...
def test_process_unknown_task_returns_404(client):
    response = client.post("/evolution/process/does-not-exist")
...
def test_progression_reflects_task_and_skill_state(client):
    ...
    client.post(f"/evolution/process/{done['id']}")

    cancelled = client.post("/tasks", json={"title": "b"}).json()
    client.patch(f"/tasks/{cancelled['id']}", json={"status": "cancelled"})
    client.post(f"/evolution/process/{cancelled['id']}")

    response = client.get("/evolution/progression")
    assert response.status_code == 200
    body = response.json()
...
def test_progression_scoped_to_project(client):
    ...
    client.post(f"/evolution/process/{task_a['id']}")

    task_b = client.post("/tasks", json={"title": "b", "project_id": "proj-2"}).json()
    client.patch(f"/tasks/{task_b['id']}", json={"status": "done"})
    client.post(f"/evolution/process/{task_b['id']}")

    response = client.get("/evolution/progression", params={"project_id": "proj-1"})
    body = response.json()
