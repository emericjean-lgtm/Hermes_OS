...
(conversation/routes.py) — path resolution, tool gating on
active_project_id, and the thin file_tools adapter, all without needing a
live model (respond_events' own tool-calling loop is Ollama's concern,
not this module's)."""
...
def test_resolve_relative_path_joins_against_root(tmp_path):
    root = tmp_path / "ws"
    root.mkdir()
    resolved = conv_routes._resolve_in_project(str(root), "AGENTS.md")
...
def test_resolve_relative_path_with_subdirectory(tmp_path):
    root = tmp_path / "ws"
    root.mkdir()
    resolved = conv_routes._resolve_in_project(str(root), "src/main.py")
...
def test_resolve_absolute_path_inside_root_is_normalized(tmp_path):
    ...
    root.mkdir()
    target = root / "f.txt"
    resolved = conv_routes._resolve_in_project(str(root), str(target))
    assert resolved == str(target.resolve())


def test_resolve_absolute_path_outside_root_passes_through_unchanged(tmp_path):
    """This function is a convenience, never the security boundary — an
    absolute path outside root is handed back as-is so Aegis's real
    whitelist check rejects it explicitly, rather than this function
    silently reinterpreting a model-supplied absolute path as relative."""
    root = tmp_path / "ws"
    root.mkdir()
    outside = str(tmp_path / "elsewhere" / "f.txt")
    assert conv_routes._resolve_in_project(str(root), outside) == outside
...
def test_resolve_relative_escape_collapses_within_join(tmp_path):
    ...
    resolved = conv_routes._resolve_in_project(str(root), "../../escape.txt")
...
# ── _execute_workspace_tool: thin adapter, no duplicated logic ──


@pytest.mark.asyncio
async def test_execute_workspace_tool_read_denied_outside_project(tmp_path):
    ...
    would raise."""
    result = await conv_routes._execute_workspace_tool(
