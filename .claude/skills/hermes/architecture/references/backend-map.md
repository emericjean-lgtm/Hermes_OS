...
- **`backend/mcp_server/`** — **live**. Hermes **as** an MCP server (inbound): **81** real tool functions (`_ALL_TOOLS`, measured 2026-09-11 — 12 files, 10 studio, 9 git, 7 memory, 7 workflows, 6 projects, 6 skills, 5 tasks, the rest across snapshots, approvals, verification and evolution), each a thin wrapper around a World-A agent, mounted at `/mcp`. This file said “~62” until HOS-289; the Tools Center said “71”, and both were right when written. The screen is now held to `_ALL_TOOLS` by a guard (`test_le_nombre_d_outils_mcp_affiche_est_celui_du_serveur`) — **this document is not**, so measure before quoting it. Built for external agent runtimes (e.g. NousResearch's "Hermes Agent") to drive Hermes.
