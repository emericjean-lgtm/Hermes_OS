...
3363 (065C) → 3437 (066C) → 3463 (067) → 3479 (068) → 3496+ (069) → 3522 (070) → 3542 (071) → 3566 (072) → 3587 (074) → 3603 (075) → 3677 (R-006) → 3703 (079) → **6190 passed / 3 skipped / 273 deselected, 0 failed (HOS-289, 2026-09-11, 7 min 18 s)**.

**Two things about the jump from 3703 to 6190.** Most of it is not new tests: HOS-111 found that `pytest.ini` was running 1190 of 4112 collected tests — 29 % — because `testpaths` named one directory and `tests/` (2594 tests, 53 % of the repo) was run by nobody. The trend above, up to 079, measures the narrow command. Read a growth figure across that boundary and it will flatter you.

The recurring flake noted here through 079 (`test_task_executor_shares_the_container_model_intelligence`, a test-ordering/shared-state issue) did **not** appear in the 2026-09-11 full run. That is one green run, not a fix: nobody has root-caused it, and a flake that does not fire is indistinguishable from a flake that was never there — see [hermes/verification](../../verification/SKILL.md) for telling a pre-existing flake from a real regression.

**This file's own trajectory.** Everything above "Standing, still-open items" describes HOS-000 → HOS-079 (2026-07-28 → 2026-08-10) and is history — it is accurate as history and should not be rewritten. But the repo is at **HOS-289** (2026-09-11), 210 milestones further on, and this file is not on the path of a roadmap pass. For anything after 08-10, `CHANGELOG.md` and `docs/HERMES_OS_ROADMAP_STATE.md` are the sources; the standing-items list below is explicitly marked "as of HOS-079" and several entries have moved since (Policy Engine, named in the HOS-041→058 line above, was audited and removed entirely in HOS-286/287).
