...
- **`components/`**:
  ...
  - `cockpit-shell.tsx` — same `views` map / `satisfies Record<string, React.FC>` guard as before (still the mechanism that turns an orphaned nav id into a compile error), now composing `Rail` + `InstrumentBar` + `StatusBar` + `CommandPalette` instead of `Sidebar` + `Topbar` + `StatusBar`. Layout no longer animates a `marginLeft` for collapse — the rail width is fixed (`--rail-w: 56px`).
