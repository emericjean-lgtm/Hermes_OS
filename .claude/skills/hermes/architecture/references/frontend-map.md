...
> **Re-measured 2026-09-11 (HOS-289).** The SODIUM description below is still accurate — tokens, fonts, geometry and texture are unchanged. Four factual claims had been overtaken by code and are corrected in place, each marked `[2026-09-11]`: the rail's pinned/expanded state, the Center count, the test counts, and the live health reading. Everything else in this file was re-checked against the tree at HOS-289 and still holds. Counts drift; where one appears below, prefer measuring it over trusting it.
...
HOS-080 replaced the entire visual system in one pass, on explicit user instruction to avoid the generic-AI-dashboard look (cyan+magenta neon, glow on every hover, uniform rounding, an undifferentiated accent palette). The organizing idea: **temperature contrast** — a cold, blue-tinted carbon chassis carrying **one** warm signal colour. Old token *names* were kept as the public API (`text-hermes-cyan`, `.glass`, `.neon-edge`, `.bracket`, etc. all still work) so the Centers inherited the new look without one edit each **([2026-09-11] 22 at the time of writing; `cockpit-shell.tsx` now mounts 25 view ids over 26 feature directories — three ids are aliases kept so an old link does not land silently on the Dashboard) — but the values underneath changed completely. Prefer the new semantic names (`sodium`/`glacier`/`steel`/`arc`/`gold`/`alarm`) in new work; the legacy names (`cyan`/`magenta`/`violet`/`green`/`amber`/`red`) are aliases kept only for backward compatibility with markup that hasn't been revisited yet.
...
**[2026-09-11] The rule is now enforced, because being stated was not enough.** Three Centers were found rendering invented figures as measured — the Security Center (HOS-288: four threats, six policies, six profiles), the System Center and the Evolution Center (HOS-289: twelve components with invented latencies, four "patterns" at 12x/85 %/+22 %, plus "42 arêtes de dépendance suivies" where the payload carries no edges). All three shared one signature: an earlier cleanup had removed the **named** mocks (`MOCK_STATUS`, `MOCK_TRUST_SCORES`), left the literal arrays inline in the JSX, and added a comment saying the mocks were gone. Searching for `MOCK_` finds nothing; nobody searches for an object array. `backend/tests/test_ecrans_sans_donnee_inventee.py` now does, over every `*-center.tsx` / `*-view.tsx`. The discriminant needs no allow-list: **invented data holds only literals, a presentation descriptor references something** — state, an interpolation, JSX, a render function. A block that genuinely describes an architecture is exempt only if a comment says so *and* no number in it reaches the rendered output.
...
- **`components/`**:
  ...
  - `rail.tsx` (new, replaces the deleted `sidebar.tsx`) — a 56px icon rail with a hover flyout for names, grouped with technical reference marks (S1…S5) instead of text headers. **[2026-09-11]** This file used to end the paragraph with "the expand/collapse sidebar is gone entirely; there is no 'collapsed' state anymore" — that is no longer true. The rail now carries a pin ("Épingler le rail déployé"): `useCockpitStore`'s `railPinned`/`toggleRailPin` widen it to `--rail-w-expanded` with labels inline. The mechanism is worth knowing before touching any shell offset — `rail.tsx` sets the `--rail-w` custom property itself, and `instrument-bar`, `statusbar` and `cockpit-shell`'s `marginLeft` all read that one property, so the whole shell moves in lockstep without any of them knowing about the pin.
...
Same vitest/jsdom setup. `__tests__/cockpit.test.ts` updated in HOS-080: the `Sidebar`/`Topbar` export checks were replaced with `Rail`/`InstrumentBar`/`CommandPalette` checks, plus a new test asserting every nav id in `nav-model.ts` is unique. 82/82 passing post-redesign.

**[2026-09-11]** Now **153 tests over 13 files**, with dedicated tests for 5 Centers (code-intelligence, governance, model-intelligence, operations, skills). The coverage gap is real but narrower than this file claimed, and the shape of it has changed: the strongest guarantee against a Center displaying something untrue is no longer a vitest file at all — it is a **backend** guard, `backend/tests/test_ecrans_sans_donnee_inventee.py`, which parses all 26 Center sources and fails when one renders an array of pure literals. See the fabricated-data rule below for why it lives there.
