...
| 13 | ✅ **Fait (HOS-229)** — relais sérialisable, quatre phases, rôle par phase | mesuré : la colonne `contrat` du J5 n'était **ni écrite ni relue**, et aucune preuve n'atteignait un prompt. `double_check` était configuré et jamais routé | 20 gardes |
| 14 | ✅ **Fait (HOS-230)** — six arrêts nommés, `inverifiable` et `cause_non_reprenable` sans user le budget, point de reprise proposé et jamais appliqué | assemblé : rien de neuf sauf la décision de continuer ou non. Ne raisonne pas, n'appelle aucun modèle — gardé sur l'AST | 20 gardes |
| 15 | ✅ **Fait (HOS-231)** — la cause voyage jusqu'au profileur ; trois causes seulement comptent contre le modèle | mesuré : `error_type` existait et personne ne l'écrivait ; un manque de VRAM abaissait la note comme une mauvaise réponse. Reste une donnée décisionnelle — **Aegis reste au-dessus** | 20 gardes |
| 16 | ✅ **Fait (HOS-232 + HOS-233)** — paquet validé, compatibilité, remplacement réel du code, self-check à dix contrôles, retour arrière code+état, état opérationnel recalculé | deux défauts trouvés : `preserve_set()` oubliait `checkpoints` **puis** `workflows`, et le `.env` de l'utilisateur vit dans l'arbre remplacé. **Hors périmètre assumé** : le téléchargement, faute de canal de distribution | 72 gardes |
| 17 | 🟨 **Partiel (HOS-234 + HOS-235)** — 8 routes servies par la **vraie** application, console de supervision, détail de run, trace réelle | ⚠️ **défaut trouvé** : les routes de HOS-234 étaient posées sur `MissionControlAPI`, **montée nulle part** — 404 sur le processus vivant. **Reste** : Control Rooms par agent, analytiques, et la vérification en navigateur (serveurs de dev antérieurs aux changements) | 44 gardes |
