...
| **G-24** | **functional** | Le contrôle natif d'ACP n'est pas émis par notre client | §16 | `acp_adapter/server.py` expose `cancel(session_id)` avec un `cancel_event` ; `hermes_agent_acp.py` ne l'émet jamais. C'est le seul chemin qui atteindrait réellement un tour de chat en cours — et il ne demande aucune convergence |
