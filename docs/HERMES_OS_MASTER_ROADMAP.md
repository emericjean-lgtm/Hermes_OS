...
| **G-16** | **technical debt** | 120 routes `/api/v1` sur 306 n'ont aucun appelant frontend (306→314 routes depuis, dette 118→113 entrées) | §15/§16 | mesuré le 2026-09-07, instrument corrigé trois fois — HOS-295 (2026-09-12) a trouvé un quatrième défaut : `_motif` n'acceptait pas `$` comme fin de chemin, si bien que `` `/route${qs}` `` (interpolation de query string, le patron dominant du client) classait 4 vrais appelants comme orphelins. Corrigé, vérifié par mutation (test rouge sans le `$`, vert avec) ; 5 entrées retirées de la dette (les 4 plus `/logs`, même trou). Le reste (113) n'est pas de la dette homogène : `/healthz`/`/readyz`/`/system/status` sont des sondes d'infra sans appelant attendu, `/legacy/*` est un pont de compatibilité documenté (P-002), `/collaboration/*` est fonctionnel mais non câblé au pipeline de mission par choix documenté (HOS-070, initiative séparée), `/studio/*` et une partie de `/git/*` ont des appelants opérateur (scripts/, pas frontend) — aucune suppression n'était étayée par une preuve suffisante ; classées à conserver, pas fantômes |
...
| 2026-09-12 | `36ccd92` | **G-10 fermé (HOS-296).** L'énoncé du gap datait d'avant une lecture directe du code : `POST /memory/{id}/promote` existait déjà (HOS-250), montée deux fois, gouvernée, testée par 19 tests — mais sans appelant frontend, exactement le défaut que G-16 (HOS-295) nomme. Panneau Quarantaine ajouté au Memory Center ; vérifié bout en bout sur le processus réel, promotion persistée après redémarrage backend. `/memory` et `/memory/{memory_id}/promote` retirés de `ORPHELINS_CONNUS` (113→111). |
