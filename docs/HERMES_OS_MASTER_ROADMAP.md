...
| approvals | ✅ 3 méthodes | ✅ négociée | ✗ **écartée** | — | 🔴 aucun producteur : file en mémoire, missions en mode déterministe, le pont ne lance aucun tour |
| steering / interruption | ✅ | ✅ négociée | ✗ | ✗ | 🟠 PLANNED — exigent un tour vivant, comme les approbations |
...
| **G-22** | **architectural** | Trois capacités attendent le chat, qui est leur seul producteur | §16 | approbations, steering et interruption exigent un tour **vivant** dans le gateway du pont. Le mécanisme d'approbation est en mémoire (`_gateway_queues`, un `threading.Event` par demande) ; les missions prennent le mode déterministe (`HERMES_SINGLE_QUERY_SESSION`). Le chat les débloque toutes les trois |
