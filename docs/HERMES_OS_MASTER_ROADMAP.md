...
| **G-23** | **architectural** | Deux transports agentiques coexistent sans passerelle | §16 | le chat utilise ACP, le pont utilise le gateway `tui_gateway`, et chacun a sa file d'approbations, ses sessions et ses contrôles. `session.steer` et `session.interrupt` sont donc inatteignables depuis le chat, et les permissions ACP invisibles du pont |
